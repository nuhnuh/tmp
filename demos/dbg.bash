...
tmux split-window -t $SESSION:teleop.1 -v # split window (creates two panes)
setup_env ${SESSION}:teleop.0
setup_env ${SESSION}:teleop.1
setup_env ${SESSION}:teleop.2
# run commands
...
tmux send-keys -t ${SESSION}:teleop.1 """rosrun tecna_turtle_bringup diff_drive_controller.py _track_width:=0.8 _wheel_radius:=0.171""" # C-m
tmux send-keys -t ${SESSION}:teleop.2 """make docker_connect""" C-m
tmux send-keys -t ${SESSION}:teleop.2 """source devel/setup.bash""" C-m
tmux send-keys -t ${SESSION}:teleop.2 """rosrun tecna_turtle_bringup diff_drive_odometry.py _track_width:=0.8 _wheel_radius:=0.171""" # C-m
# select pane
...
tmux send-keys -t ${SESSION}:rviz.0 """rosrun rviz rviz  -d dbg.rviz""" C-m
