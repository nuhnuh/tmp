...
# run commands
tmux send-keys -t ${SESSION}:rviz.0 """rviz -d \$(rospack find tecna_turtle_description)/rviz/tecna_turtle.rviz""" C-m

# plc
# create windows and panes
tmux new-window -t ${SESSION} -n plc
tmux split-window -t ${SESSION}:plc.0 -h # split window (creates two panes)
setup_env ${SESSION}:plc.0
setup_env ${SESSION}:plc.1
# run commands
tmux send-keys -t ${SESSION}:plc.0 """cd tmp/pymodbus; \
    PYTHONPATH=. python examples/common/synchronous_server.py""" C-m
tmux send-keys -t ${SESSION}:plc.1 """rosrun tecna_turtle_bringup plc.py \
    _ip:=127.0.0.1 _port:=5020 \
    _track_width:=1 _wheel_radius:=.1""" C-m
# select pane
tmux select-pane -t $SESSION:plc.0
